...
window.addEventListener('load', () => {
    // Đọc số lượt xem hiện tại từ database hiển thị lên trước khi bấm nút
...
function startPortfolio() {
    ...
    // Thực hiện tăng tổng lượt xem và ghi nhận lại địa chỉ IP
    increaseVisitorCount(); 
}

// Hàm tăng lượt truy cập đám mây bảo mật kết hợp lưu địa chỉ IP
function increaseVisitorCount() {
    // 1. Tăng bộ đếm tổng số lượt xem (+1) an toàn bằng transaction
    ...
    // 2. Gọi API để lấy IP và lưu lịch sử truy cập
    fetch('https://api.ipify.org?format=json')
        .then(response => response.json())
        .then(data => {
            const userIP = data.ip; // Lấy IP thành công

            // Tạo thời gian hiện tại theo định dạng (Năm-Tháng-Ngày Giờ:Phút:Giây)
            const now = new Date();
            ...
            // Đẩy IP và Thời gian vào nhánh 'visitor_logs' trên Firebase
            const logsRef = database.ref('visitor_logs');
            logsRef.push({
                ip: userIP,
        ...
        .catch(error => {
            // Đề phòng trường hợp người dùng bật chặn quảng cáo hoặc lỗi mạng không lấy được IP
            console.log("Không lấy được IP, ghi nhận ẩn danh.");
            const logsRef = database.ref('visitor_logs');
            logsRef.push({
                ip: "Ẩn danh (Blocked/VPN)",
